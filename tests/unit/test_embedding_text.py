...
B10.B: Function is now async — takes async_repo (AsyncMemoryRepository)
instead of sync repo. Tests use AsyncMock and await.
"""
...
@pytest.mark.asyncio
async def test_evil1_no_observations() -> None:
    """New entity with no observations → returns name + type + description only."""
    async_repo = AsyncMock()
    async_repo.get_observations_for_entity.return_value = []

    result = await _compute_entity_embedding_text(
        async_repo,
        entity_id="abc-123",
        name="TestEntity",
        node_type="Concept",
    ...
    async_repo.get_observations_for_entity.assert_called_once_with("abc-123", limit=20)
    assert result == "TestEntity Concept A test"


@pytest.mark.asyncio
async def test_evil2_empty_description() -> None:
    ...
    result = await _compute_entity_embedding_text(
        async_repo, entity_id="abc-123", name="MyEntity", node_type="Person", description=""
    )
...
@pytest.mark.asyncio
async def test_evil3_long_observation_truncated() -> None:
    """Observation > 500 chars → truncated at limit."""
    async_repo = AsyncMock()
    long_content = "x" * 1000
    async_repo.get_observations_for_entity.return_value = [{"content": long_content}]

    result = await _compute_entity_embedding_text(
        async_repo, entity_id="abc-123", name="E", node_type="T", description="D"
    )
...
@pytest.mark.asyncio
async def test_evil4_many_observations_capped() -> None:
    """50 observations → only first 20 included."""
    async_repo = AsyncMock()
    async_repo.get_observations_for_entity.return_value = [
        {"content": f"obs_{i}"} for i in range(50)
    ]
...
@pytest.mark.asyncio
async def test_sad1_none_entity_id() -> None:
    """entity_id=None → skips observation fetch entirely."""
    async_repo = AsyncMock()

    result = await _compute_entity_embedding_text(
        async_repo, entity_id=None, name="NewEntity", node_type="Thing", description="Fresh"
    )

    async_repo.get_observations_for_entity.assert_not_called()
...
@pytest.mark.asyncio
async def test_happy_full_context() -> None:
    ...
    async_repo.get_observations_for_entity.assert_called_once_with("user-001", limit=20)
