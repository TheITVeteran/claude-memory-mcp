...
class SearchAdvancedMixin:
    ...
    async def get_hologram(self, params: "GetHologramParams") -> dict[str, Any]:
        ...
        # 2. Expand Subgraph
        hologram = await self.repo.get_subgraph(anchor_ids, params.depth)  # type: ignore[attr-defined]
    ...
    async def semantic_radar(
        ...
        # Fetch source entity
        source_node = await self.repo.get_node(entity_id)  # type: ignore[attr-defined]
        if not source_node:
        ...
        for candidate in similar:
            ...
            graph_dist = await self.repo.shortest_path_length(entity_id, cid)  # type: ignore[attr-defined]
